...
DEFAULT_MEMORIES = [
    ...
    # Steins;Gate 世界观（作为“作品设定知识”，便于与用户聊梗/设定）
    "我了解《Steins;Gate/命运石之门》世界观设定：这是虚构作品中的时间旅行与世界线概念体系。",
    "世界线（Worldline）是作品中描述历史/因果分支的一种抽象；当过去发生改变，世界会切换到另一条世界线（作品设定）。",
    "发散率（Divergence）是作品中用于量化世界线差异的指标；发散率越接近表示世界线越相似（作品设定）。",
    "吸引域（Attractor Field）与“收束（Convergence）”用于描述：许多世界线会在关键事件上趋同，即便细节被改变，某些结果仍会以别的方式发生（作品设定）。",
    "α（Alpha）与 β（Beta）世界线/吸引域是作品中常见的两大分支概念：同一事件可能因所在吸引域不同而走向不同结局（作品设定）。",
    "D-mail（D-Mail）是作品中一种“向过去发送信息”的手段，会改变过去并导致世界线偏移（作品设定）。",
    "时间机器（Time Machine）在作品中既可以指发送信息的装置，也可以指能够进行时间跳跃或物理移动的装置，不同实现对应不同风险与限制（作品设定）。",
    "电话微波炉（Phone Microwave）是作品中的关键装置，与 D-mail 的实现有关（作品设定）。",
    "Reading Steiner 是作品中的一个能力设定：当世界线发生变动时，个体仍能保留旧世界线的记忆或感知差异（作品设定）。",
    "SERN（作品设定中的组织）与时间机器研究、世界线收束、监控与追踪等情节要素相关（作品设定）。",
    "ECHELON 在作品设定中与信息监控/数据收集相关，是推动事件发展的关键要素之一（作品设定）。",
    "Rounder（圆桌骑士/组织成员）在作品中通常指 SERN 相关的执行者/人员体系（作品设定）。",
    "约翰·提托（John Titor）在作品设定中与时间旅行相关，并提到 IBM 5100（原作常写作 IBN 5100）在某些任务中的关键作用（作品设定）。",
    "IBN/IBM 5100 在作品设定中被赋予“可用于处理/解析某些系统或数据”的关键能力，是多条线索汇聚点之一（作品设定）。",
]
